...
submission_views <- list(
  task1 = c(view = "syn36378025", basenames = "syn36397657")
)
...
for (task_n in seq_along(submission_views)) {
  ...
  all_scores <- lapply(1:nrow(sub_df), function(i) {
    ...
    if (!"dataset" %in% colnames(score_df)) return(data.frame())
    if (length(setdiff(score_df$dataset, pred_filenames)) > 0) return(data.frame())
  ...
  status <- lapply(1:nrow(rank_df), function(j) {
    tryCatch({
      ...
      annots <- list(primary_rank = as.double(rank_df$avg_primary_rank[j]),
                     secondary_rank = as.double(rank_df$avg_secondard_rank[j]),
